...
#> Rows: 2,382
#> Columns: 13
#> $ id                  <chr> "A009_P1_LAT_LE_M01", "A009_P1_LAT_LE_M01", "A009_…
...
|----|----|----|
| `add_articles(data)` | Adds information identifying the source article for each specific shift | `article`, `doi_url`, `id_bioshifts_v1`, `id_core`, `reference` |
...
| `add_methods(data)` | Shifts are measured using various methods which may affect the variability of detected shift rates. This function adds methodological parameters by which each individual shift was detected | `sublocality_or_pop`, `start_firstperiod`, `midpoint_firstperiod`, `end_firstperiod`, `start_lastperiod`, `midpoint_lastperiod`, `end_lastperiod`, `n_periods`, `duration`, `grain_size`, `sampling`, `category`, `obs_type`, `uncertainty_distribution`, `position_definition`, `position_definition_category` |
| `add_taxo(data)` | Shifts are reported with the species name as reported by authors (`sp_name_publication`) and the species name, checked for validity and harmonized (`sp_name_checked`). This function adds additional taxonomic information for each species. | `kingdom`, `phylum`, `class`, `family`, `taxon_id` (standard identifier), `taxon_method` |
| `add_baselines(data, type, stat, res, suffix)` | Adds the average temperature value within the study area (or the species-specific study area), over the study duration. | Variable combinations of statistic formatted as `baseline_temp_{stat}` (e.g., `baseline_temp_mean`). Also `baseline_res` for the spatial resolution over which variables were calculated. See “Adding Climate Variables” for details. If suffix is TRUE, input raster resolution will be appended to the column name. |
| `add_trends(data, type, stat, res, suffix)` | Adds the average change per year of temperature within the study area (or species-specific study area) over the study duration. See “Adding climate Variables” vignette for more. | Variable combinations of `trend_temp_{stat}` (e.g., `trend_temp_mean`) for all requested trends, and `trend_res`. If suffix is TRUE, input raster resolution will be appended to the column name. |
| `add_cv(data, type, stat, res, suffix)` | Adds the velocity of isotherm shifts over space in the latitudinal or elevational directions within study area or species-specific study area over the study duration. See “Adding Climate Variables” vignette for more. | Variable combinations of `cv_temp_{stat}` (e.g., `cv_temp_mean`) for all requested climate velocities, `along_gradient`, `cv_temp_var`, and `cv_res`. See “Adding Climate Variables” vignette for more. If suffix is TRUE, input raster resolution will be appended to the column name. |
...
merging of sub-dataframes to correctly match the shifts dataframe with
