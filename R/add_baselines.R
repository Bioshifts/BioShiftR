...
#' @param res Calculation resolution. Baseline temperatures in each species/study area were calculated with environmental raster layers at up to four resolutions: 1km, 25km, 50km, 110km, resulting in slightly different values. Choose a specific resolution with one of: `"1km"`, `"25km"`, `"50km"`, `"110km"`.
...
add_baselines <- function(data,
  ...
  # add baselines to to bioshifts
  baselines2 <- switch(type,
      ...
      .f = ~ {
        out <- data_split[[.x]] |>
          dplyr::left_join(
            baselines |>
              dplyr::select(
                article_id, poly_id, type, method_id,
                dplyr::all_of(cols[[.x]])
              ),
            by = dplyr::join_by(
              article_id, poly_id,
      ...
      .f = ~ {
        out <- data_split[[.x]] |>
          dplyr::left_join(
            ...
            by = dplyr::join_by(
              article_id, poly_id, type,
              method_id, sp_name_checked
            )
          )
        if (suffix == FALSE) {
          out <- out |>
            dplyr::mutate(baseline_res = res[[.x]]) |>
            dplyr::rename_with(
              ~ stringr::str_replace(.x, "_res.*", ""),
              dplyr::all_of(cols[[.x]])
            )
        } else if (suffix == TRUE) {
          out <- out |>
            dplyr::rename_with(
              ~ stringr::str_replace(.x, "_res", "_"),
              dplyr::all_of(cols[[.x]])
            )
        }
        out
  ...
  if (type == "SP") {
    ...
    if (n_missing > 0) {
      warning(call. = FALSE, paste0(
        "Not all shifts have associated species-specific polygon values. ",
        scales::comma(n_missing), " NAs returned."
      ))
    }
  }
